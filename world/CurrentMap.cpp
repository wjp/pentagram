		...
		fast_x_max(-1), fast_y_max(-1)
{
	...
	fast = new uint32_t*[MAP_NUM_CHUNKS];
	for (unsigned int i = 0; i < MAP_NUM_CHUNKS; i++) {
		items[i] = new list<Item*>[MAP_NUM_CHUNKS];
		fast[i] = new uint32_t[MAP_NUM_CHUNKS/32];
		std::memset(fast[i],false,sizeof(uint32_t)*MAP_NUM_CHUNKS/32);
	}
...
uint32_t CurrentMap::getNum() const
{
	if (current_map == 0)
		return 0;
...
void CurrentMap::addItem(Item* item)
{
	...
	int32_t cx = ix / mapChunkSize;
	int32_t cy = iy / mapChunkSize;
...
void CurrentMap::removeItemFromList(Item* item, int32_t oldx, int32_t oldy)
{
	...
	int32_t cx = oldx / mapChunkSize;
	int32_t cy = oldy / mapChunkSize;
...
static inline bool ChunkOnScreen(int32_t cx, int32_t cy, int32_t sleft, int32_t stop, int32_t sright, int32_t sbot, int mapChunkSize)
{
	int32_t scx = (cx*mapChunkSize - cy*mapChunkSize)/4;
	int32_t scy = ((cx*mapChunkSize + cy*mapChunkSize)/8);

	// Screenspace bounding box left extent    (LNT x coord)
	int32_t cxleft = scx-mapChunkSize/4;
	// Screenspace bounding box right extent   (RFT x coord)
	int32_t cxright= scx+mapChunkSize/4;

	// Screenspace bounding box top extent     (LFT y coord)
	int32_t cytop = scy - 256;
	// Screenspace bounding box bottom extent  (RNB y coord)
	int32_t cybot = scy + 128;
...
static inline void CalcFastAreaLimits( int32_t &sx_limit, 
										int32_t &sy_limit, 
										int32_t &xy_limit,
...
void CurrentMap::updateFastArea(int32_t from_x, int32_t from_y, int32_t from_z, int32_t to_x, int32_t to_y, int32_t to_z)
{
	int x_min = from_x;
	int x_max = to_x;
	...
	int32_t sleft  = ((x_min - y_min)/4)         - (dims.w/2 + mapChunkSize/4);
	int32_t stop   = ((x_min + y_min)/8 - z_max) - (dims.h/2 + mapChunkSize/8);
	int32_t sright = ((x_max - y_max)/4)         + (dims.w/2 + mapChunkSize/4);
	int32_t sbot   = ((x_max + y_max)/8 - z_min) + (dims.h/2 + mapChunkSize/8);
	...
	for (int32_t cy = 0; cy < MAP_NUM_CHUNKS; cy++) {
		for (int32_t cx = 0; cx < MAP_NUM_CHUNKS; cx++) {
...
void CurrentMap::setChunkFast(int32_t cx, int32_t cy)
{
...
void CurrentMap::unsetChunkFast(int32_t cx, int32_t cy)
{
	fast[cy][cx/32] &= ~(1<<(cx&31));
...
void CurrentMap::areaSearch(UCList* itemlist, const uint8_t* loopscript,
							uint32_t scriptsize, Item* check, uint16_t range,
							bool recurse, int32_t x, int32_t y)
{
	int32_t z;
	int32_t xd = 0, yd = 0, zd = 0;
...
void CurrentMap::surfaceSearch(UCList* itemlist, const uint8_t* loopscript,
					uint32_t scriptsize, Item* check, bool above, bool below,
					bool recurse)
{
	int32_t origin[3];
	int32_t dims[3];
...
void CurrentMap::surfaceSearch(UCList* itemlist, const uint8_t* loopscript,
					uint32_t scriptsize, ObjId check,
					int32_t origin[3], int32_t dims[3],
...
TeleportEgg* CurrentMap::findDestination(uint16_t id)
{
	for (unsigned int i = 0; i < MAP_NUM_CHUNKS; i++) {
		for (unsigned int j = 0; j < MAP_NUM_CHUNKS; j++) {
			item_list::iterator iter;
...
bool CurrentMap::isValidPosition(int32_t x, int32_t y, int32_t z,
								 uint32_t shape,
								 ObjId item, Item** support, uint16_t* roof)
{
	int xd, yd, zd;
...
bool CurrentMap::isValidPosition(int32_t x, int32_t y, int32_t z,
								 ...
								 uint32_t shapeflags,
								 ObjId item_, Item** support_, uint16_t* roof_)
{
	return isValidPosition(x,y,z,
						   INT_MAX/2,INT_MAX/2,INT_MAX/2,
						   xd,yd,zd,
...
bool CurrentMap::isValidPosition(int32_t x, int32_t y, int32_t z,
								 ...
								 ObjId item_, Item** support_, uint16_t* roof_)
{
	const uint32_t flagmask = (ShapeInfo::SI_SOLID | ShapeInfo::SI_DAMAGING |
							 ShapeInfo::SI_ROOF);
	const uint32_t blockflagmask = (ShapeInfo::SI_SOLID|ShapeInfo::SI_DAMAGING);

	bool valid = true;
	Item* support = 0;
	ObjId roof = 0;
	int32_t roofz = 1 << 24; //!! semi-constant
...
bool CurrentMap::scanForValidPosition(int32_t x, int32_t y, int32_t z, Item* item,
									  ...
									  int32_t& tx, int32_t& ty, int32_t& tz)
{
	...
	uint32_t blockflagmask = (ShapeInfo::SI_SOLID | ShapeInfo::SI_DAMAGING);
	static uint32_t validmask[17];
	static uint32_t supportmask[17];
...
bool CurrentMap::sweepTest(const int32_t start[3], const int32_t end[3],
	...
	for (int cx = minx; cx <= maxx; cx++) {
		for (int cy = miny; cy <= maxy; cy++) {
				 ...
				 iter != items[cx][cy].end(); ++iter)
			{
				...
				uint32_t othershapeflags = other_item->getShapeInfo()->flags;
				...
				int32_t u_0[3] = {0x4000,0x4000,0x4000}; // CONSTANTS
				...
				for( long i=0 ; i<3 ; i++ )
				{
					int32_t A_max = ext[i];	
					int32_t A_min = -ext[i];	
					int32_t B_max = other[i]+oext[i];	
					int32_t B_min = other[i]-oext[i];	
...
Item *CurrentMap::traceTopItem(int32_t x, int32_t y, int32_t ztop, int32_t zbot, ObjId ignore, uint32_t shflags)
{
	Item* top = 0;
	...
	for (int cx = minx; cx <= maxx; cx++) {
		for (int cy = miny; cy <= maxy; cy++) {
				 ...
				 iter != items[cx][cy].end(); ++iter)
			{
				...
				if (top) {
					int32_t tix, tiy, tiz, tixd, tiyd, tizd;
...
bool CurrentMap::load(IDataSource* ids, uint32_t version)
{
	for (unsigned int i = 0; i < MAP_NUM_CHUNKS; ++i) {
		for (unsigned int j = 0; j < MAP_NUM_CHUNKS/32; ++j) {
			fast[i][j] = ids->read4();
...
uint32_t CurrentMap::I_canExistAt(const uint8_t* args, unsigned int /*argsize*/)
{
	ARG_uint16_t(shape);
	ARG_uint16_t(x);
	ARG_uint16_t(y);
	...
	ARG_uint16_t(unk1); // is either 1 or 4
	ARG_uint16_t(unk2); // looks like it could be an objid
	ARG_uint16_t(unk3); // always zero
